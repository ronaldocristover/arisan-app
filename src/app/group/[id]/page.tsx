...
export default function DetailGroupPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      const groupDetail = await fetch(
        "http://localhost:8888/api/groups/06f52897-66fe-4aa6-9704-846615874dfd"
      );
      const groupDetailData = await groupDetail.json();
      setGroupData(groupDetailData.data);
      setActiveTab(0); // Set default active tab to the first one
      setLoading(false);
    }
    fetchData();
  }, []);
    ...
    <>
      ...
      <div className="bg-white shadow-md rounded-xl p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-700">
          <div>
            <p>
              <span className="font-semibold">Nama Arisan:</span>{" "}
              {groupData.name}
            </p>
            <p>
            ...
            <p>
              <span className="font-semibold">Nominal Iuran:</span> Rp{" "}
              {groupData.amount}
            </p>
          </div>
          <div>
            <p>
              <span className="font-semibold">Total Periode:</span> 12 Bulan
            </p>
            <p>
              <span className="font-semibold">Tanggal Mulai:</span>{" "}
              {groupData.start_at
                ? dayjs(groupData.start_at).format("D MMMM YYYY")
                : "-"}
            </p>
            <p>
              <span className="font-semibold">Status:</span>{" "}
              <span className="inline-block bg-green-100 text-green-700 px-2 py-1 rounded text-xs font-medium">
                {groupData.status === "ACTIVE" ? "Aktif" : "Tidak Aktif"}
              </span>
            </p>
          </div>
        </div>
